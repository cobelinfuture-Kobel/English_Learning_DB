...
def replace_once(text: str, old: str, new: str, code: str) -> str:
    if old not in text:
        raise SystemExit(f"anchor missing: {code}")
    return text.replace(old, new, 1)


builder_path = Path("ulga/builders/build_a1fs_v1_r5_local_edge_runtime_complete_evidence_collector.py")
...
''',
    '''                    "response_sha256": digest(response), "response_time_ms": response_time_ms,
                    ...
                    "outcome": outcome, "score": score, "validity_status": r1.VALID,
''',
    "r5 response event observability",
)
text = replace_once(
    text,
...
''',
    '''                    "template_family": item["template_family"],
                    ...
                    "response": json.loads(row["response_json"]),
''',
    "r5 export entry fields",
)
builder_path.write_text(text, encoding="utf-8")

validator_path = Path("ulga/validators/validate_a1fs_v1_r5_local_edge_runtime_complete_evidence_collector.py")
...
''',
    '''        if row["event_hash"] != calculated:
        ...
        previous = row["event_hash"]
''',
    "r5 validator response event observability",
)
text = replace_once(
    text,
...
''',
    ...
    _safe_scan(safe, errors)
''',
    "r5 export observability validation",
)
validator_path.write_text(text, encoding="utf-8")

workflow_path = Path(".github/workflows/a1fs-v1-r5-local-edge-runtime.yml")
...
''',
    '''          tests/ulga/test_a1fs_v1_r5_local_edge_runtime_complete_evidence_collector.py
          ...
          tests/ulga/test_a1fs_v1_r4_central_question_supply_skill_projection_capacity_governance.py
''',
    "r5 workflow regression commands",
)
workflow_path.write_text(text, encoding="utf-8")

test_path = Path("tests/ulga/test_a1fs_v1_r5_observability_upgrade.py")
