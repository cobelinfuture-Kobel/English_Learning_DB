...
from product.a1fs_v1_2_1 import u01qb16e_runtime_server as product_runtime
...
def _item_json(stimulus: str, *, correct: str = "an") -> str:
    return json.dumps(
        {
...
def _catalog_row(item_id: str, asset_key: str, stimulus: str) -> tuple[object, ...]:
    return (
        item_id,
        asset_key,
        "UNIT01-READING",
...
def _consumer_database(tmp_path: Path, *, candidate_recent: bool = False) -> Path:
    database = tmp_path / ("recent.sqlite3" if candidate_recent else "consumer.sqlite3")
    with sqlite3.connect(database) as connection:
        ...
        for index in range(1, 10):
            filler = _catalog_row(
                ...
                f"A learner can see ___ apple number {index}.",
            )
            connection.execute("INSERT INTO u01qb02_item_catalog VALUES(?,?,?,?,?,?,?,?,?,?,?,?)", filler)
        connection.execute(
        ...
        failed_signature = u16.learner_visible_signature(
            {
                "item_id": "ITEM-FAIL",
                "skill": "READING",
                "private_item_json": failed[10],
            }
        )
        different_signature = u16.learner_visible_signature(
            {
                "item_id": "ITEM-DIFFERENT",
                "skill": "READING",
                "private_item_json": candidate[10],
            }
        )
        connection.execute(
...
def test_product_manifest_uses_u01qb16e_runtime_and_installs_in_place() -> None:
    manifest = json.loads(
        (Path(product_runtime.__file__).with_name("product_manifest.json")).read_text(encoding="utf-8")
    )
    assert manifest["serve_module"] == "product.a1fs_v1_2_1.u01qb16e_runtime_server"
    ...
    assert product_runtime.e2e.impl.U01QB15ProductApplication.start_u01qb15_form is u16e._start_form_after_reassessment_gate
    assert product_runtime.e2e.impl.U01QB15ProductApplication.submit_u01qb15_response is u16e._submit_form_response_attempt_once
    assert u16e.A1FS_CONTENT_POLICY_MODE == "NOT_CONTENT_PRODUCER"
    assert u16e.A1FS_CONTENT_POLICY_EXEMPTION


def test_pending_reassessment_materializes_one_distinct_existing_item_without_exposing_fillers(
    tmp_path: Path,
) -> None:
    database = _consumer_database(tmp_path)
    ...
    assert pending[0]["active_reassessment_session"] is False
    ...
    with sqlite3.connect(database) as connection:
        ...
        sessions = connection.execute(
            f"SELECT diagnosis_id,reassessment_item_id,consumer_state FROM {u16e.SESSION_TABLE}"
        ).fetchall()
    assert len(plan_items) == 10
    assert plan_items[0] == (1, "ITEM-DIFFERENT", "REMEDIATION")
    assert all(row[1] != "ITEM-FAIL" for row in plan_items)
    assert bindings == [
        (
            "ACTIVITY-1",
            "ITEM-DIFFERENT",
    ...
    assert sessions == [("DIAG-1", "ITEM-DIFFERENT", "ACTIVE")]


def test_recent_candidate_fails_closed_instead_of_replaying_it(tmp_path: Path) -> None:
    database = _consumer_database(tmp_path, candidate_recent=True)
    with pytest.raises(u16e.DifferentItemReassessmentError, match="REASSESSMENT_CANDIDATE_RECENTLY_EXPOSED"):
...
def test_attempt_once_gate_allows_diagnosis_after_fail_but_blocks_pending_or_missing(
    ...
    monkeypatch.setattr(u16e, "_ORIGINAL_READINESS", lambda _database, _session: dict(base))
    value = u16e._completion_readiness_attempt_once(Path("unused"), "SESSION")
    assert value["completion_allowed"] is True
    assert value["same_item_retry_allowed"] is False
    assert value["different_item_reassessment_required_count"] == 1
    ...
    monkeypatch.setattr(
        ...
        lambda _database, _session: {
            "skill": "READING",
            "assets": [{"completion_state": "PENDING_HUMAN_REVIEW"}],
        },
    )
    assert u16e._completion_readiness_attempt_once(Path("unused"), "SESSION")["completion_allowed"] is False
    monkeypatch.setattr(
        u16e,
        "_ORIGINAL_READINESS",
        lambda _database, _session: {
            "skill": "READING",
            "assets": [{"completion_state": "NOT_ATTEMPTED"}],
        },
    )
    assert u16e._completion_readiness_attempt_once(Path("unused"), "SESSION")["completion_allowed"] is False


def test_same_item_second_attempt_is_rejected_before_m6_capture(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    database = tmp_path / "attempt_once.sqlite3"
    with sqlite3.connect(database) as connection:
        connection.executescript(
            """
        ...
        connection.commit()
    monkeypatch.setattr(u16e, "_ORIGINAL_SUBMIT_FORM_RESPONSE", lambda _self, _payload: {"unexpected": True})
    app = SimpleNamespace(database_path=database)
    with pytest.raises(u16e.DifferentItemReassessmentError, match="UNIT01_SAME_ITEM_RETRY_FORBIDDEN"):
