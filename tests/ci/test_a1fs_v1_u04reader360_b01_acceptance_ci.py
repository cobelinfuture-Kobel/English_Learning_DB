...
def test_u04_reader360_b06_e151_e180_cumulative_acceptance() -> None:
    report=reader.build_acceptance_report()
    assert report["status"]==reader.STATUS
    assert report["materialized_start"]=="U04-NEB-E004"
    assert report["materialized_end"]=="U04-NEB-E180"
    assert report["materialized_episode_count"]==177
    assert report["latest_batch_start"]=="U04-NEB-E151"
    assert report["latest_batch_end"]=="U04-NEB-E180"
    assert report["latest_batch_episode_count"]==30
    assert report["spoken_entry_count"]==177
    assert report["pattern_entry_count"]==177
    assert report["current360_passage_alignment_count"]==354
    assert report["current360_metadata_alignment_count"]==354
    assert report["spoken_relation_alignment_count"]==177
    assert report["pattern_family_semantic_count"]==1239
