...
def test_u04_reader360_b09_e241_e270_cumulative_acceptance() -> None:
    report=reader.build_acceptance_report()
    assert report["status"]==reader.STATUS
    assert report["materialized_start"]=="U04-NEB-E004"
    assert report["materialized_end"]=="U04-NEB-E270"
    assert report["materialized_episode_count"]==267
    assert report["latest_batch_start"]=="U04-NEB-E241"
    assert report["latest_batch_end"]=="U04-NEB-E270"
    assert report["latest_batch_episode_count"]==30
    assert report["spoken_entry_count"]==267
    assert report["pattern_entry_count"]==267
    assert report["current360_passage_alignment_count"]==534
    assert report["current360_metadata_alignment_count"]==534
    assert report["spoken_relation_alignment_count"]==267
    assert report["pattern_family_semantic_count"]==1869
