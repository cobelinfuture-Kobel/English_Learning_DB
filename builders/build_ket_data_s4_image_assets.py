...
def materialize_private_assets(source_dir, output_dir, root=None, write_pngs=True) -> dict:
    try: import fitz
    ...
    m = build_structural_inventory(root); source_dir, output_dir = Path(source_dir), Path(output_dir)
...
def main() -> None:
    p = argparse.ArgumentParser(); p.add_argument("--root"); p.add_argument("--source-dir"); p.add_argument("--output-dir"); p.add_argument("--manifest-out", required=True); p.add_argument("--structural-only", action="store_true"); p.add_argument("--no-write-pngs", action="store_true"); a = p.parse_args()
    if a.structural_only: m = build_structural_inventory(a.root)
        ...
        m = materialize_private_assets(a.source_dir, a.output_dir, a.root, not a.no_write_pngs)
