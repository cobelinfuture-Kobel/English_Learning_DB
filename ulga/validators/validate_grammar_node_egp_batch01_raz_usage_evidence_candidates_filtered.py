...
EXPECTED_TASK = "R7-M75A_Batch01RAZUsageEvidenceQualityFilterImplementation"
EXPECTED_IDS = {
...
def validate():
    print("Validating filtered Batch 01 RAZ usage evidence candidates...")
    ...
    for record in records:
        raw_count = record.get("raw_candidate_count")
        filtered_count = record.get("filtered_candidate_count")
        candidates = record.get("candidates", [])
        if raw_count < filtered_count:
            return fail("raw count below filtered count")
        if filtered_count != len(candidates):
            return fail("filtered count mismatch inside record")
        raw_total += raw_count
        filtered_total += filtered_count
        if not candidates:
            missing += 1
        for candidate in candidates:
            key = (record.get("grammar_id"), candidate.get("sentence_text", "").strip().lower(), candidate.get("matched_text", "").strip().lower())
            if key in seen:
                return fail("duplicate filtered candidate present")
            seen.add(key)
            if candidate.get("source_type") != "RAZ":
                return fail("candidate source_type must be RAZ")
            if candidate.get("quality_filter_status") != "KEPT":
                return fail("filtered candidate must be marked KEPT")
    ...
    if summary.get("raw_candidate_count") != raw_total:
        return fail("raw_candidate_count mismatch")
    if summary.get("filtered_candidate_count") != filtered_total:
        return fail("filtered_candidate_count mismatch")
    if summary.get("removed_candidate_count") != raw_total - filtered_total:
        return fail("removed_candidate_count mismatch")
    if summary.get("targets_without_candidates") != missing:
        return fail("targets_without_candidates mismatch")
    ...
    if summary.get("next_short_step") != "R7-M76A_Batch01RAZUsageEvidenceQualityFilterReadback":
        return fail("next_short_step mismatch")
    if summary.get("stop_reason") != "NONE":
        return fail("stop_reason must be NONE")
    print("Filtered Batch 01 RAZ usage evidence candidate validation: PASS")
    return True
