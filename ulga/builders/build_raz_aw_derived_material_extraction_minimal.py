...
The source contract follows the local 197-JSON reality audit:
...
FORBIDDEN_SAFE_KEYS = {
    "text", "clean_text", "raw_text", "source_text", "original_text",
    "title", "sentence", "sentences", "passage", "transcript",
...
def _theme_labels_from_record(record: Mapping[str, Any]) -> set[str]:
    """Read only actual theme labels; exclude confidence and source metadata."""
    result = _strings(record.get("candidate_theme_tags"))
    tags = record.get("theme_tags")
    if isinstance(tags, Mapping):
        for key in ("primary_theme", "mapped_theme", "subthemes"):
            result.update(_strings(tags.get(key)))
    return {
...
def _candidate_refs(record: Mapping[str, Any]) -> dict[str, set[str]]:
    return {
        ...
        "grammar": _strings(record.get("candidate_grammar_refs")),
    }
...
def load_direct_metadata(
    ...
    direct_refs = {name: set() for name in ("vocabulary", "chunks", "patterns", "grammar")}
    ...
    for filename in filenames:
        ...
        rows = _records(path)
        for row in rows:
            themes.update(_theme_labels_from_record(row))
    ...
    for level in levels:
        ...
        for row in rows:
            ...
            if not ref or ref in seen:
                raise MinimalExtractionError(f"invalid_or_duplicate_page_unit_ref:{level}:{ref}")
        ...
        source_index.append({
            "level": level,
            "path": "|".join(path.relative_to(source_root).as_posix() for path in semantic_paths),
            "record_count": len(rows),
            "derived_schema": derived_schema,
            "sha256": deep.sha256_value([deep.sha256_file(path) for path in semantic_paths]),
        })
...
def _scene_structure(
    ...
    if semantic["situation_families"] & {
        "science_observation", "nature_observation", "animals_and_habitats"
    }:
...
def _four_skill_affordances(
    ...
    if discourse in {
        "sequence", "cause_effect", "comparison_or_contrast",
...
def build_package(
    ...
    *,
    ...
    level_state = {
        level: {
            "refs": set(), "books": set(),
            "themes": set(direct_metadata[level]["theme_labels"]),
            "pedagogy": set(direct_metadata[level]["pedagogy_labels"]),
            "families": set(), "micro": set(), "functions": set(),
            "roles": set(), "goals": set(),
            "vocabulary": set(), "chunks": set(), "patterns": set(), "grammar": set(),
            "seed_counts": Counter(), "discourse_counts": Counter(),
            "scene_counts": Counter(), "skill_counts": Counter(),
    ...
    for row in page_units:
        ...
        grammar = deep.grammar_units(text, manifest_tags.get(ref, set()))
        ...
        maturity = _seed_maturity(grammar, vocabulary, chunks, patterns, discourse, sentence_count)
        duplicate_group = f"SDUP_{deep.sha256_value(deep.normalize(text))[:16].upper()}"
        row_themes = _theme_labels_from_record(row)

        state = level_state[level]
        state["refs"].add(ref)
        state["books"].add(book_id)
        state["themes"].update(row_themes)
        ...
        evidence.append({
            ...
            "theme_labels": sorted(row_themes),
            ...
            "matched_grammar_refs": sorted(grammar),
    ...
    for level in levels:
        ...
        per_level.append({
            ...
            "theme_labels": sorted(state["themes"]),
            ...
            "direct_candidate_vocabulary_refs": direct["direct_candidate_vocabulary_refs"],
            "direct_candidate_chunk_refs": direct["direct_candidate_chunk_refs"],
            "direct_candidate_pattern_refs": direct["direct_candidate_pattern_refs"],
            "direct_candidate_grammar_refs": direct["direct_candidate_grammar_refs"],
            ...
            "matched_grammar_refs": sorted(state["grammar"]),
            "direct_pedagogy_labels": sorted(state["pedagogy"]),
            "sentence_seed_counts": dict(sorted(state["seed_counts"].items())),
            "semantic_duplicate_group_count": len(state["duplicate_groups"]),
            "discourse_shape_counts": dict(sorted(state["discourse_counts"].items())),
            "scene_structure_counts": dict(sorted(state["scene_counts"].items())),
            "four_skill_affordance_counts": dict(sorted(state["skill_counts"].items())),
        })
    ...
    checks = {
        ...
        "all_page_units_have_situation": all(row["situation_families"] and row["micro_situations"] for row in evidence),
        "all_page_units_have_sentence_seed": all(row["sentence_seed_maturity"] for row in evidence),
        "all_page_units_have_scene_structure": all(row["scene_structure"] for row in evidence),
        "all_page_units_have_skill_affordance": all(row["four_skill_affordances"] for row in evidence),
        "all_scene_evidence_text_derived": all(row["scene_evidence_status"] == "DERIVED_SCENE_STRUCTURE" for row in evidence),
    }
    ...
    package: dict[str, Any] = {
        ...
        "source_scope": {
            ...
            "source_files": sorted([dict(row) for row in source_index], key=lambda row: (row["level"], row["path"])),
        },
        "authority_baselines": {
            name: {
        ...
        "page_unit_evidence": sorted(evidence, key=lambda row: row["source_unit_ref"]),
        "aggregate_summary": {
            "theme_label_count": len(aggregate("theme_labels")),
            "theme_labels": sorted(aggregate("theme_labels")),
            "situation_family_count": len(aggregate("situation_families")),
            "micro_situation_count": len(aggregate("micro_situations")),
            "communicative_function_count": len(aggregate("communicative_functions")),
            ...
            "matched_grammar_ref_count": len(aggregate("matched_grammar_refs")),
            "semantic_duplicate_group_count": len({row["semantic_duplicate_group_id"] for row in evidence}),
            "passage_seed_count": sum(row["passage_seed_status"] == "SUPPORTED" for row in evidence),
            "direct_image_evidence_count": 0,
        },
        "extraction_gate": {
            "source_checks": checks,
            "decision": "DERIVED_MATERIAL_READY_FOR_LOCAL_VALIDATION" if ready else "BLOCKED_DERIVED_SOURCE_OR_ACCOUNTING",
            "ready_for_governance_binding": False,
            "ready_for_canonical_promotion": False,
            "ready_for_learning_unit_population": False,
            "next_short_step": "RAZ-AW_DerivedMaterialExtractionLocalReplay",
        },
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if leakage:
            raise MinimalExtractionError("safe_output_leakage:" + ";".join(leakage[:20]))
