#!/usr/bin/env python3
"""Build safe content, exercise, and scene candidate envelopes for 24 A1/A1+ units."""
...
A1FS_CONTENT_POLICY_EXEMPTION = "Metadata-only candidate envelopes over admitted item IDs, promoted RAZ material IDs, and source-proven Theme/Situation refs; no learner content or private source payload is produced."
...
def _verify_cp03(artifact: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    checks = (
        (artifact.get("task_id") == cp03.TASK_ID, "cp03_task_id_mismatch"),
        (artifact.get("schema_version") == cp03.SCHEMA_VERSION, "cp03_schema_version_mismatch"),
        (artifact.get("scope") == "A1_A1_PLUS_ONLY", "cp03_scope_mismatch"),
        (artifact.get("stop_reason") == "NONE", "cp03_not_passed"),
        (artifact.get("next_short_step") == TASK_ID, "cp03_next_short_step_mismatch"),
    )
    ...
    grammar_ids = [str(row.get("grammar_unit_id") or "") for row in units]
    learning_ids = [str(row.get("learning_unit_id") or "") for row in units]
    if (
        "" in grammar_ids
        or "" in learning_ids
        or len(set(grammar_ids)) != 24
        or len(set(learning_ids)) != 24
        or [row.get("sequence_index") for row in units] != list(range(1, 25))
    ):
        raise CandidateBuildError("cp03_learning_unit_identity_invalid")
    return units


def _m11b_candidates(unit: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    learning_id = str(unit["learning_unit_id"])
    binding = unit.get("m11b_reviewed_content_binding", {})
    by_skill = binding.get("admitted_item_ids_by_skill", {})
    if not isinstance(by_skill, Mapping) or set(by_skill) != set(SKILLS):
        raise CandidateBuildError(f"m11b_skill_partition_invalid:{learning_id}")
    ...
    for skill in SKILLS:
        ...
        if not isinstance(item_ids, list) or len(item_ids) != len(set(item_ids)):
            raise CandidateBuildError(f"m11b_item_ids_invalid:{learning_id}:{skill}")
        for raw_id in item_ids:
            item_id = str(raw_id or "")
            if not item_id or item_id in seen:
                raise CandidateBuildError(f"m11b_item_identity_invalid:{learning_id}:{skill}")
            seen.add(item_id)
            content_id = _candidate_id("CONTENT_M11B", learning_id, skill, item_id)
            content.append(
                {
                    "content_candidate_id": content_id,
                    "source_kind": "M11B_REVIEWED_SHARED_ITEM",
                    "source_ref": item_id,
                    "source_task_id": binding.get("source_task_id"),
                    "skill": skill,
            ...
            exercises.append(
                {
                    "exercise_candidate_id": _candidate_id(
                        "EXERCISE_M11B", learning_id, skill, item_id
                    ),
                    "content_candidate_id": content_id,
                    "source_kind": "M11B_REVIEWED_SHARED_ITEM",
                    "source_ref": item_id,
...
def _raz_candidates(unit: Mapping[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    learning_id = str(unit["learning_unit_id"])
    grammar_id = str(unit["grammar_unit_id"])
    materials = unit.get("raz_admitted_asset_binding", {}).get("materials")
    if not isinstance(materials, list):
        raise CandidateBuildError(f"raz_material_partition_invalid:{learning_id}")
    content: list[dict[str, Any]] = []
    exercises: list[dict[str, Any]] = []
    seen: set[str] = set()
    for material in materials:
        if not isinstance(material, Mapping):
            raise CandidateBuildError(f"raz_material_row_invalid:{learning_id}")
        ...
        if (
            ...
            or material.get("grammar_authority_ref") != grammar_id
            or material.get("registry_status") != "PROMOTED_TO_A1_A1PLUS_MATERIAL_REGISTRY"
            or scope not in {"A1", "A1_PLUS"}
        ):
            raise CandidateBuildError(f"raz_material_binding_invalid:{learning_id}:{material_id}")
        seen.add(material_id)
        content_id = _candidate_id("CONTENT_RAZ", learning_id, material_id)
        content.append(
            {
                "content_candidate_id": content_id,
        ...
        exercises.append(
            {
                "exercise_candidate_id": _candidate_id("EXERCISE_RAZ", learning_id, material_id),
                "content_candidate_id": content_id,
                "source_kind": "RAZ_PROMOTED_MATERIAL",
                "source_ref": material_id,
                "target_skill_lanes": [],
...
def _scene_candidates(unit: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(refs, list) or len(refs) != len(set(refs)):
        raise CandidateBuildError(f"scene_authority_refs_invalid:{learning_id}")
    if (refs and status != "SELECTED_AUTHORITY_BACKED") or (
        not refs and status != "PENDING_SOURCE_EVIDENCE"
    ):
        raise CandidateBuildError(f"scene_authority_status_invalid:{learning_id}")
    return [
        {
            "scene_candidate_id": _candidate_id("SCENE", learning_id, str(ref)),
...
def build_artifact(cp03_artifact: Mapping[str, Any]) -> dict[str, Any]:
    ...
    m11b_count = raz_binding_count = scene_count = scene_units = 0
    units_with_content = complete_units = 0
    ...
    for unit in units:
        ...
        local_content_ids = {row["content_candidate_id"] for row in content}
        local_exercise_ids = {row["exercise_candidate_id"] for row in exercises}
        local_scene_ids = {row["scene_candidate_id"] for row in scenes}
        if (
            len(local_content_ids) != len(content)
            or len(local_exercise_ids) != len(exercises)
            or len(local_scene_ids) != len(scenes)
            or content_ids & local_content_ids
            or exercise_ids & local_exercise_ids
            or scene_ids & local_scene_ids
        ):
            raise CandidateBuildError("candidate_id_collision")
        content_ids |= local_content_ids
        exercise_ids |= local_exercise_ids
        scene_ids |= local_scene_ids
        unit_m11b_refs = {row["source_ref"] for row in m11b_content}
        ...
        scene_units += bool(scenes)
        units_with_content += bool(content)
        complete_units += bool(content and exercises and scenes)
        output_units.append(
            {
    ...
    summary = {
        ...
        "content_candidate_count": len(content_ids),
        "exercise_candidate_count": len(exercise_ids),
        ...
        "candidate_envelope_complete_unit_count": complete_units,
    }
    artifact = {
        "task_id": TASK_ID,
