...
The existing five canonical Unit01 contexts plus the approved 27 U01QB07
model-authored scene specifications remain the only scene sources.  This module
creates a read-only dereference view keyed by ``scene_ref_id`` so downstream
rotation/allocation/runtime layers no longer need to reconstruct partial scene
meaning from their intentionally-small projections.
"""
...
A1FS_CONTENT_POLICY_EXEMPTION = (
    "Read-only canonical resolver over existing approved Unit01 scene and language "
...
def _spec() -> dict[str, Any]:
    ...
    if len(u07.candidates(value)) != EXPECTED_MODEL_SCENE_COUNT:
        raise CanonicalMicroSceneAuthorityError("MODEL_SCENE_COUNT_INVALID")
    return value
...
def _model_scene_rows() -> list[dict[str, Any]]:
    # The five fixed Unit01 contexts are approved seed authorities and cover the
    # object inventory used by the already-approved U01QB07 supplement.  Reusing
    # U01QB07's own materializer recovers semantic core + resolved seed lineage.
    anchors = _canonical_context_rows()
    return [u07.model_scene_row(candidate, anchors) for candidate in u07.candidates(_spec())]
...
def _scene_language_projection(package: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for sentence in authority["sentences"]:
        words = _words(sentence.get("text"))
        if str(sentence.get("context_id") or "") == ref or (
            bool(words & object_words) and bool(words & semantic_words)
        ):
...
def _canonical_package(context: Mapping[str, Any], row: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return _finish_package({
        ...
        "scene_core": core,
        # Canonical context builder already fixes IDENTIFY + DESCRIBE and
        # FIRST_MENTION + KNOWN_REFERENCE; this is a semantic identifier, not
        # newly-authored learner content.
