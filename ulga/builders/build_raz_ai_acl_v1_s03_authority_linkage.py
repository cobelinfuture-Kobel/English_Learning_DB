...
The public S03 contract remains compatible with S04-S06, but every Theme,
Vocabulary, Chunk, Pattern, and Grammar ref is now checked against its real
registry.  The three operator-approved Theme candidates are translated to
canonical Theme IDs only after those IDs exist in the Theme catalog.
"""
...
def _grammar_registry(
    ...
    return ids, {
        ...
        "canonical_egp_row_count": len(
            {ref for refs in mapping.values() for ref in refs}
        ),
    }, mapping
...
def load_authority_registry(
    *,
    ...
    authorities = deep.load_authorities()
    ...
    registry = {
        "THEME": themes,
        "VOCABULARY": set(authorities["vocabulary"]["ids"]),
        "CHUNK": set(authorities["chunks"]["ids"]),
        "PATTERN": set(authorities["patterns"]["ids"]),
        "GRAMMAR": grammar,
    }
    identity = {
        "THEME": theme_identity,
        "VOCABULARY": {
            "source_path": authorities["vocabulary"]["source_path"],
            "source_sha256": authorities["vocabulary"]["source_sha256"],
            "id_count": len(registry["VOCABULARY"]),
        },
        "CHUNK": {
            "source_path": authorities["chunks"]["source_path"],
            "source_sha256": authorities["chunks"]["source_sha256"],
            "id_count": len(registry["CHUNK"]),
        },
        "PATTERN": {
            "source_path": authorities["patterns"]["source_path"],
            "source_sha256": authorities["patterns"]["source_sha256"],
...
def build_package(
    ...
    *,
    ...
    canonical_target_sets: dict[str, set[str]] = {
        key: set() for key in expected_types
    }
    ...
    for row in sorted(
        ...
        for authority_type, field in REFERENCE_FIELDS:
            ...
            for source_authority_ref in _list_of_strings(row, field):
                ...
                canonical_target_sets[authority_type].add(canonical_ref)
                links.append(
                    {
    ...
    package: dict[str, Any] = {
        ...
        "aggregate_summary": {
            ...
            "unique_canonical_authority_ref_counts": {
                key: len(values)
                for key, values in sorted(canonical_target_sets.items())
            },
