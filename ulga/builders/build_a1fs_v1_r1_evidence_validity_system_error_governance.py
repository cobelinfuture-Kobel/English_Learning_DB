#!/usr/bin/env python3
"""Govern A1FS evidence validity without rewriting raw learner attempts.

Task: A1FS-V1-R1_EvidenceValidityAndSystemErrorGovernance

The source database remains the immutable evidence authority.  This module adds an
append-only validity event chain and a current validity projection, then creates a
private governed database overlay containing only VALID attempts.  Existing M7
mastery/remediation and M8 retention engines consume that overlay unchanged.
"""
...
CREATE TABLE IF NOT EXISTS evidence_validity_events(
  ...
  attempt_id TEXT NOT NULL REFERENCES response_attempts(attempt_id),
...
CREATE TABLE IF NOT EXISTS evidence_validity(
  attempt_id TEXT PRIMARY KEY REFERENCES response_attempts(attempt_id),
  validity_status TEXT NOT NULL,
  latest_event_id TEXT REFERENCES evidence_validity_events(event_id),
  reason_code TEXT NOT NULL,
  detail_json TEXT NOT NULL,
  actor_id TEXT NOT NULL,
...
def _require_source(connection: sqlite3.Connection) -> None:
    ...
    required = {"response_attempts", "scoring_results", "human_review_queue"}
    ...
    with _open(database_path) as connection:
        ...
        for row in attempts:
            connection.execute(
                ...
                VALUES(?,?,?,?,?,?,?,?)""",
                (row["attempt_id"], VALID, None, "CAPTURED_AS_VALID", "{}", "SYSTEM", at, row["attempt_hash"]),
            )
        projection_count = connection.execute("SELECT COUNT(*) FROM evidence_validity").fetchone()[0]
        if projection_count != len(attempts):
            raise EvidenceGovernanceError("validity_projection_attempt_count_mismatch")
        metadata = {
            "r1_task_id": TASK_ID,
            "r1_schema_version": SCHEMA_VERSION,
            "r1_validation_status": STATUS,
            "r1_raw_attempt_rewrite_enabled": "false",
            "r1_invalid_evidence_mastery_eligible": "false",
            "r1_next_short_step": NEXT_SHORT_STEP,
        }
        connection.executemany("INSERT OR REPLACE INTO metadata(key,value) VALUES(?,?)", metadata.items())
        connection.commit()
    return {
        "validation_status": STATUS,
        "attempt_count": projection_count,
        "raw_attempt_rewrite_enabled": False,
    ...
    *,
    ...
    with _open(database_path) as connection:
        ...
        attempt = connection.execute(
            "SELECT attempt_hash FROM response_attempts WHERE attempt_id=?", (attempt_id,)
        ).fetchone()
        if not attempt:
            raise EvidenceGovernanceError("attempt_not_found")
        current = connection.execute(
            "SELECT * FROM evidence_validity WHERE attempt_id=?", (attempt_id,)
        ).fetchone()
        if not current:
        ...
        previous = connection.execute(
            "SELECT event_hash FROM evidence_validity_events ORDER BY event_seq DESC LIMIT 1"
        ).fetchone()
        ...
        core = {
            ...
            "actor_id": actor_id,
            "occurred_at": at,
            "source_attempt_hash": attempt["attempt_hash"],
        }
        event_hash = digest(previous_hash + canonical(core))
        connection.execute(
            """INSERT INTO evidence_validity_events
            (event_id,attempt_id,previous_status,new_status,reason_code,detail_json,actor_id,occurred_at,source_attempt_hash,previous_hash,event_hash)
            VALUES(?,?,?,?,?,?,?,?,?,?,?)""",
            (
                event_id, attempt_id, previous_status, new_status, reason_code,
                canonical(detail_value), actor_id, at, attempt["attempt_hash"], previous_hash, event_hash,
            ),
        )
        connection.execute(
...
def _rebuild_attempt_chain(connection: sqlite3.Connection) -> None:
    previous_hash = "0" * 64
    rows = connection.execute("SELECT rowid,* FROM response_attempts ORDER BY rowid").fetchall()
    for row in rows:
        core = {
        ...
        connection.execute(
            "UPDATE response_attempts SET previous_hash=?,attempt_hash=? WHERE attempt_id=?",
            (previous_hash, attempt_hash, row["attempt_id"]),
        )
...
def build_governed_overlay(
    ...
    *,
    ...
    source_database_path = Path(database_path)
    governed_database_path = Path(governed_database_path)
    report_path = Path(report_path)
    if governed_database_path.resolve() == source_database_path.resolve():
        raise EvidenceGovernanceError("governed_database_must_not_replace_source")
    governed_database_path.parent.mkdir(parents=True, exist_ok=True)
    report_path.parent.mkdir(parents=True, exist_ok=True)
    temporary = governed_database_path.with_suffix(governed_database_path.suffix + ".tmp")
    if temporary.exists():
        temporary.unlink()

    with _open(source_database_path) as source:
        _require_source(source)
        rows = source.execute(
            """SELECT v.attempt_id,v.validity_status,v.reason_code,v.detail_json,v.actor_id,v.updated_at,
            v.source_attempt_hash,a.attempt_hash FROM evidence_validity v JOIN response_attempts a USING(attempt_id)
            ORDER BY a.rowid"""
        ).fetchall()
    ...
    with _open(temporary) as governed:
        ...
        excluded_ids = [row["attempt_id"] for row in excluded]
        for attempt_id in excluded_ids:
    ...
    os.replace(temporary, governed_database_path)
    try:
        os.chmod(governed_database_path, 0o600)
    except OSError:
        pass

    status_counts: dict[str, int] = {status: 0 for status in sorted(VALIDITY_STATUSES)}
    for row in rows:
    ...
    report_core = {
        ...
        "built_at": at,
        "source_database_sha256": file_digest(source_database_path),
        "governed_database_sha256": file_digest(governed_database_path),
        "source_attempt_count": len(source_ids),
        "effective_attempt_count": len(effective_ids),
        ...
        "claim_boundaries": {
            "raw_attempts_rewritten": False,
            "scoring_outcomes_rewritten": False,
            "mastery_policy_relaxed": False,
            "canonical_graph_modified": False,
            "a2_unlocked": False,
            "qwen_required": False,
            "audio_population_required": False,
        },
        "next_short_step": NEXT_SHORT_STEP,
    }
    ...
    return {
        "validation_status": STATUS,
        "governed_database_path": str(governed_database_path),
...
def main() -> int:
    ...
    change.add_argument("--attempt-id", required=True)
    change.add_argument("--status", required=True)
    change.add_argument("--reason-code", required=True)
    change.add_argument("--actor-id", required=True)
    change.add_argument("--detail-json", default="{}")
    overlay = commands.add_parser("build-overlay")
    overlay.add_argument("--database", type=Path, required=True)
    overlay.add_argument("--governed-database", type=Path, required=True)
