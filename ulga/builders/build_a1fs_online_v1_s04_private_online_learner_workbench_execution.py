...
class WorkbenchApplication:
    ...
    def start_session(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        session_id = str(payload.get("session_id") or f"A1FS_ONLINE_V1_S04_SESSION:{skill.upper()}:{uuid.uuid4().hex}")
    ...
    def submit_response(self, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if payload.get("attempt_id"):
            kwargs["attempt_id"] = str(payload["attempt_id"])
        if payload.get("submitted_at"):
            kwargs["submitted_at"] = str(payload["submitted_at"])
        result = self.response_store.capture_response(**kwargs)
        with sqlite3.connect(self.database_path) as connection:
            version = int(connection.execute(
                "SELECT session_version FROM learning_sessions WHERE session_id=?",
                (str(payload["session_id"]),),
            ).fetchone()[0])
        return {**result, "session_version": version}
...
def _select_canary_response(database_path: Path, reading_assets: Sequence[Mapping[str, Any]]) -> tuple[str, Any]:
    asset_ids = [str(asset["asset_key"]) for asset in reading_assets]
    with sqlite3.connect(database_path) as connection:
        rows = connection.execute(
            "SELECT asset_key,contract_json FROM response_contracts WHERE lesson_id=(SELECT lesson_id FROM lesson_assets WHERE asset_key=? LIMIT 1) AND capture_enabled=1",
            (asset_ids[0],),
        ).fetchall()
    contracts = {str(asset_key): json.loads(contract_json) for asset_key, contract_json in rows}
    for asset_key in asset_ids:
        ...
        if mode in {"EXACT_OPTION", "NORMALIZED_TEXT"}:
            return asset_key, "__intentional_s04_canary_wrong_answer__"
        if mode == "EXACT_SEQUENCE":
            return asset_key, ["__intentional_s04_canary_wrong_token__"]
    raise WorkbenchError("deterministic_reading_canary_contract_missing")
...
def run_http_canary(*, app: WorkbenchApplication, static_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        asset_key, canary_response = _select_canary_response(app.database_path, reading["assets"])
        ...
        result = _request(port, "POST", "/api/response", {
            ...
            "asset_key": asset_key, "response": canary_response,
        ...
        completed = _request(port, "POST", "/api/session/complete", {
            "session_id": CANARY_SESSION_ID, "expected_session_version": result["session_version"],
