#!/usr/bin/env python3
"""Admit Unit01 RAZ-grounded scene assets and bind them to existing three-skill authority."""
...
REVIEW_DIMENSIONS = (
    "GRAMMAR_SAFETY", "VOCABULARY_SAFETY", "SEMANTIC_NATURALNESS",
    "A1_ANSWERABILITY", "SCENE_DISTINCTNESS", "THREE_SKILL_AFFORDANCE",
)
FUTURE_ROLES = (
    "PREREQUISITE", "CARRY_OVER", "RECOMBINATION", "TRANSFER",
    "SCHEDULED_REVIEW", "REMEDIATION", "ASSESSMENT_SUPPORT",
)
REUSE_GATES = (
    "PREREQUISITE_UNLOCKED", "LEVEL_SCOPE_ALLOWED",
    "NEW_GRAMMAR_COMPATIBILITY_PASS", "NO_UNINTRODUCED_GRAMMAR",
    "SEMANTIC_COMPATIBILITY_PASS", "SCENE_DEDUPLICATION_PASS",
...
def validate_upstream(report: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if scope.get("allowed_units") != [UNIT_ID] or scope.get("canonical_promotion") is not False or scope.get("a2_status") != "LOCKED":
...
def validate_decision(decision: Mapping[str, Any], candidate: Mapping[str, Any]) -> None:
    if decision.get("source_record_id") != candidate.get("source_record_id") or decision.get("semantic_identity") != candidate.get("semantic_identity"):
        raise AdmissionBuildError("DECISION_IDENTITY_MISMATCH")
    if decision.get("decision_ref") != DECISION_REF or decision.get("review_status") not in {"APPROVED", "REJECTED"}:
...
def content_parts(decision: Mapping[str, Any]) -> tuple[list[str], list[dict[str, str]]]:
    ...
    sentences = [str(value).strip() for value in decision.get("adapted_sentences") or [] if str(value).strip()]
    turns = [
        {"speaker_id": str(value.get("speaker_id") or ""), "utterance": str(value.get("utterance") or "").strip()}
        for value in decision.get("dialogue_turns") or [] if isinstance(value, Mapping)
    ]
    ...
    if kind == "SHORT_DIALOGUE":
        ...
        if sentences or not 2 <= len(turns) <= 6 or len(speakers) < 2 or any(not turn["speaker_id"] or not turn["utterance"] for turn in turns):
...
def build_asset(candidate: Mapping[str, Any], decision: Mapping[str, Any], contract: Mapping[str, Any]) -> dict[str, Any]:
    ...
    required_scene_fields = {"setting", "participants", "objects", "actions", "information_structure", "communicative_function_ids"}
    ...
    vocabulary_rows = list(contract["vocabulary_contract"]["active_vocabulary"]) + list(contract["vocabulary_contract"]["active_adjectives"])
    wanted = set(candidate.get("active_noun_hits") or []) | set(candidate.get("active_adjective_hits") or [])
    vocabulary_ids = sorted(str(row["evp_sense_id"]) for row in vocabulary_rows if row["lemma"] in wanted)
    ...
    for skill in SKILLS:
        ...
        modes = ["SHORT_TEXT_DETAIL", "ARTICLE_REFERENCE"] if skill == "READING" else ["GUIDED_SENTENCE", "CONTEXTUAL_WRITING"] if skill == "WRITING" else ["ROLE_PLAY", "ORAL_RETELL"] if kind == "SHORT_DIALOGUE" else ["ORAL_RETELL"]
        projections.append({
            "projection_id": f"{content_asset_id}-{skill}", "content_asset_id": content_asset_id,
            "skill": skill, "existing_question_bank_id": qb.BANK_ID,
            ...
            "projection_status": "READY_FOR_EXISTING_QB_MATERIALIZATION", "task_modes": modes,
        })

    speakers = sorted({turn["speaker_id"] for turn in turns})
    return {
        "content_asset_id": content_asset_id, "content_kind": kind,
        "title": str(decision.get("title") or content_asset_id),
        "introduced_unit_id": UNIT_ID, "introduced_unit_sequence": 1,
        "source_lineage": {
            "source_authority": "RAZ_READING_AUTHORITY", "source_record_id": str(candidate["source_record_id"]),
            "semantic_identity": str(candidate["semantic_identity"]), "source_level": candidate.get("source_level"),
            "source_type": candidate.get("source_type"),
            "original_excerpt_sha256": hashlib.sha256(raw_excerpt.encode("utf-8")).hexdigest(),
            "original_excerpt_private": True, "adaptation_mode": str(decision.get("adaptation_mode") or ""),
            "adaptation_reason_codes": sorted(str(value) for value in decision.get("adaptation_reason_codes") or []),
            "derived_from_task_id": upstream.TASK_ID,
        },
        "content": content, "content_sha256": digest(content),
        "target_alignment": {
            "grammar_target_ids": pattern_ids,
            "egp_row_ids": sorted(list(contract["grammar_contract"]["core_focus_egp_row_ids"]) + list(contract["grammar_contract"]["guided_extension_egp_row_ids"])),
            "vocabulary_asset_ids": vocabulary_ids, "chunk_asset_ids": [],
            "sentence_frame_ids": sorted(str(value) for value in candidate.get("matched_sentence_frame_ids") or []),
            "theme_id": decision.get("theme_id"), "situation_family_id": decision.get("situation_family_id"),
            "micro_situation_id": decision.get("micro_situation_id"),
            "communicative_function_ids": sorted(str(value) for value in scene["communicative_function_ids"]),
        },
        "scene_profile": scene,
        "dialogue_profile": {
            "is_real_dialogue": kind == "SHORT_DIALOGUE", "speaker_count": len(speakers),
            "turn_count": len(turns), "speaker_ids": speakers,
            "adjacency_pair_types": sorted(str(value) for value in decision.get("adjacency_pair_types") or []),
            "role_play_supported": kind == "SHORT_DIALOGUE",
        },
        "skill_projections": projections,
        "admission": {
            "review_status": "APPROVED", "decision_ref": DECISION_REF,
        ...
        "later_unit_reuse": {
            "reusable_in_later_units": True, "reuse_identity_mode": "REFERENCE_EXISTING_CONTENT_ASSET_ID",
            "copy_on_reuse": False, "eligible_future_unit_roles": list(FUTURE_ROLES),
            "reuse_gates": list(REUSE_GATES),
        },
        "unit02_reusable_handoff": {
            "target_unit_sequence": TARGET_UNIT02_SEQUENCE, "source_content_asset_id": content_asset_id,
            "candidate_role": "CARRY_OVER", "binding_status": "AVAILABLE_NOT_BOUND",
            "unit02_modified": False,
            "required_when_bound": ["target_unit_id", "target_unit_role", "new_grammar_target_ids", "reuse_reason", "compatibility_gate_status"],
        },
    }
...
def build_payload(selection_report: Mapping[str, Any], decisions: Mapping[str, Any], contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    by_id = {str(row.get("source_record_id") or ""): row for row in decision_rows if isinstance(row, Mapping)}
    if "" in by_id or len(by_id) != len(decision_rows) or set(by_id) != {str(row["source_record_id"]) for row in reviewable}:
        raise AdmissionBuildError("COMPLETE_REVIEWABLE_CANDIDATE_DECISIONS_REQUIRED")

    assets, ledger = [], []
    for candidate in reviewable:
        decision = by_id[str(candidate["source_record_id"])]
        validate_decision(decision, candidate)
        ledger.append({
            "source_record_id": candidate["source_record_id"], "semantic_identity": candidate["semantic_identity"],
            "review_status": decision["review_status"], "decision_ref": decision["decision_ref"],
            "content_kind": decision.get("content_kind"),
            "rejection_reason_codes": sorted(str(value) for value in decision.get("rejection_reason_codes") or []),
        })
        if decision["review_status"] == "APPROVED":
            assets.append(build_asset(candidate, decision, contract))
    counts = Counter(asset["content_kind"] for asset in assets)
    if not assets or any(counts[kind] < 1 for kind in CONTENT_KINDS):
    ...
    return {
        ...
        "scope": {
            "allowed_units": [UNIT_ID], "unit02_to_unit24_modified": False, "a2_status": "LOCKED",
            "listening_status": "DEFERRED", "second_question_bank_created": False,
            "raw_raz_text_learner_facing_copy_allowed": False,
        },
        "inputs": {
            "upstream_task_id": upstream.TASK_ID, "approved_contract_sha256": APPROVED_CONTRACT_SHA256,
            "existing_question_bank_id": qb.BANK_ID, "existing_question_bank_version": qb.BANK_VERSION,
            "decision_ref": DECISION_REF,
        },
        "inspection_record": {
            "inspection_ref": INSPECTION_REF,
            "findings": [{"finding_code": code, "observed_status": "CONFIRMED", "evidence": evidence} for code, evidence in FINDINGS],
        ...
        "coverage_readback": {
            "reviewable_candidate_count": len(reviewable), "approved_content_asset_count": len(assets),
            "rejected_candidate_count": sum(row["review_status"] == "REJECTED" for row in ledger),
            "distinct_micro_scene_count": counts["MICRO_SCENE"],
            "distinct_short_passage_count": counts["SHORT_PASSAGE"],
            "distinct_dialogue_count": counts["SHORT_DIALOGUE"],
            "raz_grounded_content_count": len(assets),
            "project_authored_rewrite_count": sum(asset["source_lineage"]["adaptation_mode"] == "PROJECT_AUTHORED_REWRITE" for asset in assets),
            "reading_projection_count": len(assets), "writing_projection_count": len(assets),
            "speaking_projection_count": len(assets), "three_skill_shared_content_count": len(assets),
            "template_only_content_count": 0, "unit02_reusable_asset_count": len(assets),
        },
        "boundaries": {
            "existing_question_bank_referenced": True, "existing_question_bank_modified": False,
            "parallel_question_bank_created": False, "unit02_modified": False, "audio_enabled": False,
            "speaking_capture_enabled": False, "mastery_claimed": False,
        },
        "next_short_step": NEXT_SHORT_STEP,
    }


def build_candidate(selection_report: Mapping[str, Any], decisions: Mapping[str, Any], contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
    return policy_artifact.build_candidate(
        payload=build_payload(selection_report, decisions, contract), producer_id=TASK_ID,
        level_scope=["A1"],
        source_bindings={
            "upstream_task_id": upstream.TASK_ID, "approved_contract_sha256": APPROVED_CONTRACT_SHA256,
            "existing_question_bank_id": qb.BANK_ID, "existing_question_bank_version": qb.BANK_VERSION,
            "operator_decision_ref": DECISION_REF, "operator_inspection_ref": INSPECTION_REF,
        },
    )


def admit_candidate(candidate: Mapping[str, Any]) -> dict[str, Any]:
    from ulga.validators import validate_a1fs_v1_razq01d_unit01_micro_scene_passage_dialogue_admission_three_skill_projection_unit02_handoff as validator
    return policy_artifact.admit_candidate(
        candidate, validation_receipts=[validator.validate_candidate(candidate)],
...
def build_safe_readback(approved: Mapping[str, Any]) -> dict[str, Any]:
    ...
    safe = {
        "schema_version": SAFE_SCHEMA_VERSION, "program_id": PROGRAM_ID, "task_id": TASK_ID,
        "status": PASS_STATUS, "approved_artifact_sha256": approved["artifact_sha256"],
        "content_governance": deepcopy(approved["content_governance"]),
        "admission": deepcopy(approved["admission"]), "source_bindings": deepcopy(approved["source_bindings"]),
        "inspection_record": deepcopy(payload.get("inspection_record") or {}),
        "coverage_readback": deepcopy(payload.get("coverage_readback") or {}),
        "content_assets": [safe_asset(asset) for asset in payload.get("content_assets") or []],
        "boundaries": deepcopy(payload.get("boundaries") or {}), "next_short_step": NEXT_SHORT_STEP,
    }
    safe["readback_sha256"] = digest(safe)
    return safe


def build_admission(selection_report: Mapping[str, Any], decisions: Mapping[str, Any], contract: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
...
def run(selection_report_path: Path, decisions_path: Path, candidate_output_path: Path = OUTPUT_CANDIDATE, approved_output_path: Path = OUTPUT_APPROVED, safe_output_path: Path = OUTPUT_SAFE) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    candidate, approved, safe = build_admission(load(selection_report_path), load(decisions_path))
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        _, approved, safe = run(args.selection_report.resolve(), args.decisions.resolve(), args.candidate_output.resolve(), args.approved_output.resolve(), args.safe_output.resolve())
    except (AdmissionBuildError, policy_artifact.ContentPolicyBuildError, ValueError, KeyError, TypeError) as exc:
