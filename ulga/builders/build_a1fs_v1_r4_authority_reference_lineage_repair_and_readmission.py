#!/usr/bin/env python3
"""Repair R4 authority-reference lineage and re-admit existing candidates.

This builder changes source-lineage metadata only. It does not author, rewrite,
or infer learner-visible content. Existing M2-derived candidates must retain an
exact asset/lesson/content-digest match. An already Authority-approved project-
authored candidate whose declared M2 digest is absent is rebound to an explicit
project-content identity derived from its unchanged candidate content.
"""
...
A1FS_CONTENT_POLICY_EXEMPTION = "Repairs source-lineage metadata and re-admits existing validated R4 candidates without authoring or changing learner-visible content."
...
REVIEW_CRITERIA = (
    "a1_a1plus_level_fit",
    "breadth_cell_fit",
    "learner_stimulus_complete",
    "answer_or_rubric_valid",
...
def _single_prefixed(values: Any, prefix: str, code: str) -> str:
    matches = [
        value[len(prefix):]
        for value in values
...
def _project_content_core(candidate: Mapping[str, Any]) -> dict[str, Any]:
    """Return the immutable project-authored content identity, excluding lineage."""
    return {
...
def _old_review(candidate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(review, Mapping) or review.get("status") != "APPROVED":
        raise AuthorityLineageRepairError(f"previous_authority_review_not_approved:{item_id}")
    if not isinstance(expected, str) or review.get("candidate_sha256") != expected:
        raise AuthorityLineageRepairError(f"previous_authority_review_not_hash_bound:{item_id}")
    if expected != r4.candidate_digest(candidate):
        raise AuthorityLineageRepairError(f"previous_authority_review_not_hash_bound:{item_id}")
    criteria = review.get("criteria")
    if not isinstance(criteria, Mapping) or any(criteria.get(key) is not True for key in REVIEW_CRITERIA):
        raise AuthorityLineageRepairError(f"previous_authority_criteria_not_pass:{item_id}")
...
def _already_repaired(
    ...
    *,
    ...
    if not isinstance(refs, list) or not isinstance(review, Mapping):
        return None
    receipt = review.get("lineage_repair")
    if not isinstance(receipt, Mapping) or receipt.get("task_id") != TASK_ID:
        return None
    if f"{M1_PREFIX}{graph_sha256}" not in refs or f"{M2_PREFIX}{consumer_sha256}" not in refs:
        return None
    if review.get("candidate_sha256") != candidate.get("candidate_sha256"):
        return None
    if candidate.get("candidate_sha256") != r4.candidate_digest(candidate):
        return None
    return str(receipt.get("resolution") or "ALREADY_REPAIRED")


def _replace_authority_refs(
    values: list[Any],
    *,
    graph_sha256: str,
    consumer_sha256: str,
    ...
    retained = [
        ...
        and value
        and not value.startswith(LINEAGE_PREFIXES)
    ]
    return [
...
def repair_candidate(
    ...
    *,
    ...
    prior_resolution = _already_repaired(
        row,
        graph_sha256=graph_sha256,
        consumer_sha256=consumer_sha256,
    )
    if prior_resolution:
        return row, prior_resolution

    old_review = _old_review(row)
    item_id = str(row.get("item_id") or "")
    if row.get("level") not in {"A1", "A1_PLUS"}:
        raise AuthorityLineageRepairError(f"a2_or_out_of_scope_candidate:{item_id}")
    source_refs = row.get("source_refs")
    authority_refs = row.get("authority_refs")
    if not isinstance(source_refs, list) or not isinstance(authority_refs, list):
        raise AuthorityLineageRepairError(f"source_or_authority_refs_missing:{item_id}")

    old_sha = str(row["candidate_sha256"])
    asset_id = _single_prefixed(source_refs, "M2_ASSET:", f"m2_asset_ref_invalid:{item_id}")
    lesson_id = _single_prefixed(source_refs, "M2_LESSON:", f"m2_lesson_ref_invalid:{item_id}")
    declared_content = _single_prefixed(
        authority_refs,
        M2_CONTENT_PREFIX,
        f"m2_content_digest_ref_invalid:{item_id}",
    )
    matches = list(assets.get((asset_id, lesson_id), []))
    if len(matches) > 1:
        raise AuthorityLineageRepairError(f"duplicate_m2_asset_binding:{item_id}")
    exact_m2_content = (
        len(matches) == 1
        and str(matches[0].get("content_digest") or "") == declared_content
    )

    provenance = str(row.get("provenance") or "")
    if provenance == "EXISTING_AUTHORITY_REVIEWED":
        if not exact_m2_content:
            raise AuthorityLineageRepairError(f"existing_authority_m2_content_mismatch:{item_id}")
        content_refs = [f"{M2_CONTENT_PREFIX}{declared_content}"]
        resolution = "M2_AUTHORITY_LINEAGE_REBOUND"
    elif provenance == "PROJECT_AUTHORED_CANDIDATE" and exact_m2_content:
        content_refs = [f"{M2_CONTENT_PREFIX}{declared_content}"]
        ...
        raise AuthorityLineageRepairError(f"provenance_invalid:{item_id}:{provenance}")

    row["authority_refs"] = _replace_authority_refs(
        authority_refs,
        graph_sha256=graph_sha256,
        consumer_sha256=consumer_sha256,
    ...
    row["authority_review"] = {
        ...
        "criteria": {key: True for key in REVIEW_CRITERIA},
...
def repair_registry(
    *,
    ...
    expected_project_resolution_count: int | None,
    ...
    if expected_item_count is not None and len(rows) != expected_item_count:
        raise AuthorityLineageRepairError(
            f"candidate_count_mismatch:{len(rows)}:{expected_item_count}"
        )
    ...
    for candidate in rows:
        ...
        repaired, resolution = repair_candidate(
            candidate,
            graph_sha256=graph_sha256,
            consumer_sha256=consumer_sha256,
            assets=assets,
    ...
    repaired_registry = deepcopy(dict(candidate_registry))
    repaired_registry["candidates"] = repaired_rows
    repaired_registry["candidates_sha256"] = r4.digest(repaired_rows)
    repaired_registry["semantic_sha256"] = r4.candidate_registry_semantic_digest(repaired_rows)
    repaired_registry["authority_lineage_repair"] = {
        "task_id": TASK_ID,
        "graph_sha256": graph_sha256,
        "consumer_sha256": consumer_sha256,
        "candidate_count": len(repaired_rows),
    }
    return repaired_registry, dict(sorted(counts.items()))
...
def build_repair(
    *,
    ...
    source_registry = read_json(candidates_path, "candidate_registry")
    ...
    temporary_path = Path(candidates_path).with_name(
        f".{Path(candidates_path).name}.authority_repair.tmp"
    )
    write_json_atomic(temporary_path, repaired_registry, private=True)
    try:
    ...
    project_count = resolution_counts.get("PROJECT_AUTHORED_CONTENT_IDENTITY_REBOUND", 0)
    report_core = {
        "task_id": TASK_ID,
        ...
        "source_bindings": {
            ...
            "ontology_sha256": repaired_registry.get("ontology_sha256"),
            "coverage_sha256": repaired_registry.get("coverage_sha256"),
        },
        "counts": {
            ...
            "project_authored_source_resolved_count": project_count,
...
def safe_scan(value: Any) -> None:
    forbidden = {
        "prompt", "context", "options", "accepted_texts", "accepted_sequence",
        "model_text", "model_texts", "rubric", "learner_contract",
        "private_scoring_contract", "response",
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--reviewer-id",
        default="OPERATOR_APPROVED_A1FS_V1_R4_LINEAGE_REPAIR",
    )
    ...
    try:
        ...
        safe_scan(supply)
        safe_scan(report)
        write_json_atomic(args.output_root / CANDIDATE_OUTPUT, registry, private=True)
        write_json_atomic(args.output_root / BANK_OUTPUT, bank, private=True)
        write_json_atomic(args.output_root / SUPPLY_OUTPUT, supply, private=False)
        write_json_atomic(args.output_root / REPORT_OUTPUT, report, private=False)
        print(json.dumps({
