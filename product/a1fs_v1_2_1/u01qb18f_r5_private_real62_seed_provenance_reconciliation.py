...
def _validate_r4_report(report: Mapping[str, Any], *, real62_sha256: str) -> None:
    ...
    if str(proof.get("real62_artifact_sha256") or "") != real62_sha256:
...
def _private_inventory(approved_content: Mapping[str, Any], *, real62_sha256: str) -> dict[str, Any]:
    ...
    return u06.build_inventory(
        approved_content,
        s01.CONTEXTS,
        approved_content_sha256=real62_sha256,
...
def materialize_reconciliation(
    *,
    ...
    r4_report_path: Path,
    output: Path,
    expected_real62_sha256: str = real62_contract.EXPECTED_REAL62_SHA256,
) -> dict[str, Any]:
    real62_path = Path(real62_path).resolve(strict=True)
    r4_report_path = Path(r4_report_path).resolve(strict=True)
    actual_sha = _file_sha256(real62_path)
    if actual_sha != expected_real62_sha256:
        raise PrivateReal62ProvenanceError(
            f"REAL62_SHA256_INVALID:{actual_sha}:{expected_real62_sha256}"
        )
    r4_report = _load_json(r4_report_path)
    _validate_r4_report(r4_report, real62_sha256=actual_sha)
    approved_content = _load_json(real62_path)
    inventory = _private_inventory(approved_content, real62_sha256=actual_sha)
    ...
    result = {
        ...
        "real62_artifact_sha256": actual_sha,
