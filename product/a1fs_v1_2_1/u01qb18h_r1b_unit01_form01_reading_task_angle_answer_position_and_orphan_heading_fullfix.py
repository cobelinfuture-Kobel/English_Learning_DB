...
The actual Form01 human review recorded three defects:
* VISUAL_ORPHAN_SCENE_HEADING
* READING_CORRECT_OPTION_COLLAPSE
* READING_TASK_ANGLE_COLLAPSE

The first CI wave exposed an important current-product fact: Form01's two Reading
angles are ARTICLE_CONTROL and KNOWN_REFERENCE_CONTEXT, not FIRST_MENTION_CONTEXT.
R1A's generic answer-demonstration suppression had removed the natural first-
mention context from KNOWN_REFERENCE_CONTEXT items, making a second-mention task
look like another context-free article blank. R1B therefore restores only that
already-selected learner context, adds learner-safe cues for the two existing
angles, and binds each scene heading to its first activity for print pagination.
No answer-side field is read, exported, rewritten, or inferred as scoring truth.
"""
...
A1FS_CONTENT_POLICY_EXEMPTION = (
    ...
    "print consumer and unchanged 474-item QuestionBank. It derives task-angle identity "
    "from the existing U01QB09 GUIDED allocation owner, restores only learner context "
    "already present on the selected KNOWN_REFERENCE_CONTEXT item, changes learner-facing "
    "cue wording and print pagination only, never reads or exports correct answers, and "
    "creates no QuestionBank item, selector, planner, runtime, database, scoring authority, "
...
PASS_STATUS = (
    ...
    "UNIT01_FORM01_READING_TASK_ANGLE_ANSWER_MEANING_ORPHAN_HEADING_FULLFIX"
)
NEXT_SHORT_STEP = (
...
def _guided_angle_plan() -> dict[str, tuple[str, ...]]:
    """Read the installed U01QB09 owner instead of duplicating task allocation."""
    support = u01qb09.support_for_form(_FORM01_ORDINAL)
    if support != "GUIDED":
        raise Form01PdfR1BError(f"FORM01_SUPPORT_DRIFT:{support}")
    plan = {
        skill: tuple(u01qb09.choose_angles(support, skill, set(), count))
        for skill, count in _EXPECTED_PER_SCENE.items()
    }
    # This is the current installed product result. The assertion is intentionally
    # fail-closed so a future curriculum/allocation change cannot silently inherit
    # this Form01-only printable repair.
    expected = {
        "READING": ("ARTICLE_CONTROL", "KNOWN_REFERENCE_CONTEXT"),
...
def _enrich_form01_task_angles(student: Mapping[str, Any]) -> dict[str, Any]:
    """Attach existing U01QB09 task-angle identity to a private review copy."""
...
def _reading_guided_cue(activity: Mapping[str, Any]) -> str:
    ...
    if angle == "KNOWN_REFERENCE_CONTEXT":
        return "Same thing again: read the context, then choose the article for the same thing."
    raise Form01PdfR1BError(
        f"FORM01_READING_TASK_ANGLE_INVALID:{activity.get('question_number')}:{angle}"
    )


def _known_reference_context(activity: Mapping[str, Any]) -> str:
    """Recover natural context that R1A suppressed as an apparent answer demo.

    A KNOWN_REFERENCE_CONTEXT item needs its first mention (for example,
    ``I can see a bag.``) so the following target phrase is interpreted as a
    repeated reference. The first sentence is not the target answer; it is the
    semantic evidence that makes the second mention answerable.
    """
    raw = str(activity.get("stimulus") or "").strip()
    target = base._target_phrase(raw)
    ...
    for raw_part in raw.split("|"):
        ...
        if any(
            ...
            for prefix in base._FORBIDDEN_PRESENTATION_PREFIXES
        ):
            continue
        value = base._humanize_inline(part)
        marker = re.search(r"target\s+phrase\s*:", value, flags=re.I)
        if marker:
            before = value[: marker.start()].strip(" .·")
            if before:
                natural_parts.append(before + ".")
            break
        if base._is_named_segment(value):
            continue
        natural_parts.append(value)

    context = " · ".join(part for part in natural_parts if part).strip()
    if not context:
        return ""
    if not base._contains_direct_article_phrase(context, target):
        return ""
    return context
...
def _clean_stimulus_r1b(activity: Mapping[str, Any]) -> str:
    ...
    angle = str(activity.get("task_angle") or "")
    if angle == "KNOWN_REFERENCE_CONTEXT":
        context = _known_reference_context(activity)
        if not context:
            raise Form01PdfR1BError(
                f"KNOWN_REFERENCE_CONTEXT_MISSING:{activity.get('question_number')}"
            )
        parts = [cue, f"Context: {context}"]
        if value:
            parts.append(value)
        return " · ".join(parts)
...
def render_form_html(student: Mapping[str, Any]) -> str:
    ...
    for engineering_id in ("article_control", "known_reference_context"):
