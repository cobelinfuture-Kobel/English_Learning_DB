...
TASK_ID = "A1FS-V1-U04R360-B03_Reader360E061E090CumulativeAcceptance"
STATUS = "PASS_A1FS_V1_U04R360_B03_E061_E090_CUMULATIVE_E004_E090"
...
EXPECTED_IDS = tuple(f"U04-NEB-E{i:03d}" for i in range(4, 91))
LATEST_BATCH_IDS = tuple(f"U04-NEB-E{i:03d}" for i in range(61, 91))
...
def _check_contract(payload:dict[str,Any],label:str)->None:
    ...
    if contract.get("current360_passage_preserved") is not True:raise Reader360BatchAcceptanceError(f"{label}_passage_preservation_contract_missing")
...
def build_acceptance_report(repo_root:Path|str|None=None)->dict[str,Any]:
    ...
    s_entries=list(spoken.get("entries",[]));p_entries=list(pattern.get("entries",[]))
    s_ids=tuple(str(r.get("source_episode_id","")) for r in s_entries);p_ids=tuple(str(r.get("source_episode_id","")) for r in p_entries)
    ...
    passage_alignment_count=pattern_family_semantic_count=spoken_relation_alignment_count=0;spoken_norms=set();pattern_norms=set()
    for srow,prow in zip(s_entries,p_entries,strict=True):
        episode_id=str(srow["source_episode_id"]);source=sources[episode_id];declared=set(_rels(source["target_relations"]))
